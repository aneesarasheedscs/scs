...
import SaleAnalycisCategories from './SaleAnalycisDetail/SaleAnalycisCategories';
...
function SalesAnalytics({ form, data, saleInvoice }: PurchaseAnalyticsDetailProps) {
  const { t } = useTranslation();

  const [IpcId, setIpcId] = useState<number | null>(null);
  const [SortNo, setSortNo] = useState<number | null>(null);
  ...
  return (
    <div style={{ marginTop: 20 }}>
      <Row style={{ marginLeft: 5 }}>
        <Col xxl={24} lg={24}>
          <Row style={{ marginLeft: 1, marginBottom: 10 }} gutter={[10, 10]}>
                          ...
                          <div>
                            ...
                            >
                              <Col xxl={5} style={{ textAlign: 'center' }}>
                                <a
                                  onClick={() => handleOpenItemGroup(item.IpcId, item.SortNo)}
                                  style={{ color: 'blue' }}
                                >
                                  <u>{item.Descriptions}</u>
                                </a>
                              </Col>
                              <Col xxl={5}>
      ...
      <>
        ...
        >
          <div style={{ maxHeight: '100%' }}>
            <SaleAnalycisCategories saleInvoice={saleInvoice} data={data} IpcId={IpcId} />
          </div>
        </Modal>
