...
function Department() {
  ...
  return (
    <AddButtonforItems open={open} handleOpen={handleOpen} handleClose={handleClose}>
      <h2 style={{ marginBottom: 20 }}>{t('department')} </h2>
      <Divider></Divider>
      ...
      <Form form={form} layout="horizontal" onFinish={handleFinish} initialValues={{ remember: true }}>
        <Row gutter={[10, 10]} style={{ marginLeft: '3%' }}>
          ...
          <Col xs={24} sm={24} md={{ span: 6, offset: 1 }} className="formfield">
            <AntSelectDynamic
              bordered={false}
              label={t('salaries_payable_ac')}
              fieldValue="Id"
              fieldLabel="CheqNo"
              name="Id"
            />
          ...
          <Col xs={24} sm={24} md={{ span: 7, offset: 1 }} className="formfield">
            <AntInput name="Equivalent" label={t('department_name')} bordered={false} />
          </Col>

          <Col xs={24} sm={24} md={{ span: 6, offset: 1 }} className="formfield">
            <AntSelectDynamic bordered={false} label={t('loan_ac')} fieldValue="Id" fieldLabel="CheqNo" name="Id" />
          </Col>
          ...
          <Col xs={24} sm={24} md={3} offset={16}>
            <AntButton
              label={t('cancel')}
              htmlType="submit"
              className="fullWidth"
