...
import { TOtherItem, TWsRmStockReceivedNotesExpensesList } from '../types';

const { useWatch } = Form;

const ExpenseEntryForm = ({ form }: TDynamicForm) => {
  const [disablefields, setDisablefields] = useState(true);
  const formValues = useWatch<TWsRmStockReceivedNotesExpensesList[]>('WsRmStockReceivedNotesExpensesList', form);
  ...
  const handleAddToTable = () => {
    ...
    setFields([
      { name: ['WsRmStockReceivedNotesExpensesList', 0, 'ItemId'], value: null },
      { name: ['WsRmStockReceivedNotesExpensesList', 0, 'OtherItemName'], value: null },
      { name: ['WsRmStockReceivedNotesExpensesList', 0, 'Rate'], value: null },
      { name: ['WsRmStockReceivedNotesExpensesList', 0, 'Qty'], value: null },
      { name: ['WsRmStockReceivedNotesExpensesList', 0, 'Amount'], value: null },
      { name: ['WsRmStockReceivedNotesExpensesList', 0, 'Remarks'], value: null },
    ]);
    setIsEditMode(false);
  };
    ...
    setExpenseTableData((prevData: any[]) => {
      ...
      if (rowIndex !== -1) {
        updatedData[rowIndex] = {
          ...updatedData[rowIndex],
          Id: record.Id,
          OtherItemName: record.OtherItemName,
        ...
        form.setFieldValue(['WsRmStockReceivedNotesExpensesList', 0], updatedData[rowIndex]); // Update form values
        setIsEditMode(true);
      }
      console.log('New tableData:', updatedData);
      return updatedData;
  ...
  const handleItemChange = (obj: TOtherItem, index: number) => {
    ...
    setFields([{ name: ['WsRmStockReceivedNotesExpensesList', index, 'OtherItemName'], value: obj?.OtherItemName }]);
  };
  const handleQtyChange = (Qty: number | string | null, index: number) => {
    const Rate = getFieldValue(['WsRmStockReceivedNotesExpensesList', index, 'Rate']);
    if (Qty && typeof Qty === 'number' && Rate) {
      const amount = calculateAmount(Qty, Rate);
      setFields([{ name: ['WsRmStockReceivedNotesExpensesList', index, 'Amount'], value: amount }]);
    } else {
      setFields([{ name: ['WsRmStockReceivedNotesExpensesList', index, 'Amount'], value: null }]);
    }
  };

  const handleRateChange = (itemRate: number | string | null, index: number) => {
    const qty = getFieldValue(['WsRmStockReceivedNotesExpensesList', index, 'Qty']);
      ...
      <Row gutter={[10, 10]} style={{ marginTop: '0%' }}>
        <Col xs={24} sm={24} md={24} lg={{ span: 24 }} xl={{ span: 24 }}>
          <Card style={{ boxShadow: '2px 4px 12px 1px gray', paddingTop: '-10%' }}>
            <Form.List name="WsRmStockReceivedNotesExpensesList" initialValue={[initialValues]}>
              {(fields, {}) => (
                <>
                  {fields.map((field) => (
                    <div
