...
import { TDetailItem, TWsRmStockTransferNotesDetailsList } from '../types';

const { useWatch } = Form;

const DynamicForm = ({ form }: TDynamicForm) => {
  const [disablefields, setDisablefields] = useState(true);
  const formValues = useWatch<TWsRmStockTransferNotesDetailsList[]>('WsRmStockTransferNotesDetailsList', form);
  const { t } = useTranslation();
  const { setFields, getFieldValue } = form;
  const equivalentRate = getFieldValue(['WsRmStockTransferNotesDetailsList', 0, 'PackEquivalent']);
  const itemId = getFieldValue(['WsRmStockTransferNotesDetailsList', 0, 'ItemId']);
  const WarehouseId = getFieldValue(['WsRmStockTransferNotesDetailsList', 0, 'WarehouseId']);
  const ItemQty = getFieldValue(['WsRmStockTransferNotesDetailsList', 0, 'ItemQty']);
  const BillWeight = getFieldValue(['WsRmStockTransferNotesDetailsList', 0, 'BillWeight']);
  const itemUomId = getFieldValue(['WsRmStockTransferNotesDetailsList', 0, 'ItemUomId']);
  ...
  const handleAddToTable = () => {
    ...
    setFields([
      { name: ['WsRmStockTransferNotesDetailsList', 0, 'Item'], value: null },
      { name: ['WsRmStockTransferNotesDetailsList', 0, 'ItemId'], value: null },
      { name: ['WsRmStockTransferNotesDetailsList', 0, 'ItemName'], value: null },
      { name: ['WsRmStockTransferNotesDetailsList', 0, 'UOMCode'], value: null },
      { name: ['WsRmStockTransferNotesDetailsList', 0, 'ItemUom'], value: null },
      { name: ['WsRmStockTransferNotesDetailsList', 0, 'PackEquivalent'], value: null },
      { name: ['WsRmStockTransferNotesDetailsList', 0, 'IssuedQty'], value: null },
      { name: ['WsRmStockTransferNotesDetailsList', 0, 'BillWeight'], value: null },
      { name: ['WsRmStockTransferNotesDetailsList', 0, 'StockWeight'], value: null },
      { name: ['WsRmStockTransferNotesDetailsList', 0, 'BillWeight'], value: null },
      { name: ['WsRmStockTransferNotesDetailsList', 0, 'ItemUomId'], value: null },
      { name: ['WsRmStockTransferNotesDetailsList', 0, 'IssuedRate'], value: null },
      { name: ['WsRmStockTransferNotesDetailsList', 0, 'ItemNetAmount'], value: null },
      { name: ['WsRmStockTransferNotesDetailsList', 0, 'RemarksDetail'], value: null },
    ]);
    setIsEditMode(false);
  };
  ...
  const handleEditRow = (record: any | any[]) => {
    ...
    setTableData((prevData: any[]) => {
      ...
      if (rowIndex !== -1) {
        ...
        form.setFieldValue(['WsRmStockTransferNotesDetailsList', 0], updatedData[rowIndex]); // Update form values
        setIsEditMode(true);
      }
      console.log('New tableData:', updatedData);
      return updatedData;
  ...
  const handleItemChange = (obj: TDetailItem, index: number) => {
    ...
    setFields([
      ...
      { name: ['WsRmStockTransferNotesDetailsList', index, 'ItemUom'], value: obj?.RateUom },
      { name: ['WsRmStockTransferNotesDetailsList', index, 'ItemId'], value: obj?.ItemId },
      { name: ['WsRmStockTransferNotesDetailsList', index, 'PackEquivalent'], value: obj?.Equivalent },
      { name: ['WsRmStockTransferNotesDetailsList', index, 'IssuedRate'], value: obj?.ItemPrice },
      { name: ['WsRmStockTransferNotesDetailsList', index, 'ItemUomId'], value: obj?.ItemUomId },
      { name: ['WsRmStockTransferNotesDetailsList', index, 'ActionTypeId'], value: 1 },
    ]);

    const IssuedQty = getFieldValue(['WsRmStockTransferNotesDetailsList', index, 'IssuedQty']);
    if (IssuedQty && obj?.Equivalent) {
      const weight = calculateWeight(IssuedQty, obj?.Equivalent);
      setFields([{ name: ['WsRmStockTransferNotesDetailsList', index, 'BillWeight'], value: weight }]);
      setFields([{ name: ['WsRmStockTransferNotesDetailsList', index, 'BillWeight'], value: weight }]);
      setFields([{ name: ['WsRmStockTransferNotesDetailsList', index, 'StockWeight'], value: weight }]);
    } else {
      setFields([{ name: ['WsRmStockTransferNotesDetailsList', index, 'BillWeight'], value: null }]);
      setFields([{ name: ['WsRmStockTransferNotesDetailsList', index, 'BillWeight'], value: null }]);
      setFields([{ name: ['WsRmStockTransferNotesDetailsList', index, 'StockWeight'], value: null }]);
    }
    const weight = getFieldValue(['WsRmStockTransferNotesDetailsList', index, 'BillWeight']);
    ...
    if (itemRate && weight && obj?.Equivalent) {
      ...
      console.log(amount);
      setFields([{ name: ['WsRmStockTransferNotesDetailsList', index, 'ItemNetAmount'], value: amount }]);
    } else {
      setFields([{ name: ['WsRmStockTransferNotesDetailsList', index, 'ItemNetAmount'], value: null }]);
    }
  };
  ...
  const handleItemRateChange = (itemRate: number | string | null, index: number) => {
    ...
    if (itemRate && typeof itemRate === 'number' && weight && equivalentRate) {
      const amount = calculateAmount(weight, equivalentRate, itemRate);
      setFields([{ name: ['WsRmStockTransferNotesDetailsList', index, 'ItemAmount'], value: amount }]);
    } else {
      setFields([{ name: ['WsRmStockTransferNotesDetailsList', index, 'ItemAmount'], value: null }]);
    }
  };

  useEffect(() => {
    const weight = getFieldValue(['WsRmStockTransferNotesDetailsList', 0, 'BillWeight']);
    if (isSuccess && data?.data?.Data?.Result !== null) {
      setFields([
        ...
        {
          name: ['WsRmStockTransferNotesDetailsList', 0, 'StockWeight'],
