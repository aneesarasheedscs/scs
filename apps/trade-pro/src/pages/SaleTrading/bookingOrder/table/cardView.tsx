...
const CardView: React.FC<{ setActiveTab: (tab: string) => void; setSelectedRecordId: (id: number) => void }> = ({
  ...
  const { data, isError, isLoading, refetch, isFetching } = useGetSaleOrder(true);
  const saleOrderHistory = data?.data?.Data?.Result;
  ...
  const [selectedCardData, setSelectedCardData] = useState<any | any>();
  // const totalRecords = data?.data?.Data?.Result.length || 0;
  const { t } = useTranslation();
  useEffect(() => {
    setRecords(saleOrderHistory);
  }, [saleOrderHistory]);
  ...
  const handleSearch = (value: any) => {
    ...
    // const filteredRecords = saleOrderHistory?.filter((record: TPurchaseOrderHistory) => {
    const filteredRecords = saleOrderHistory?.filter((record: any) => {
              ...
              <Row align="middle">
                <Col xs={16} sm={16} className="columns">
                  <div className="main-voucher-design" id="Rice_Invoice_Main_Box">
                    ...
                    <Tablefile selectedRecordId={selectedRecordId} saleOrderHistory={saleOrderHistory} />
                  </div>
                </Col>
              </Row>
            </Col>
