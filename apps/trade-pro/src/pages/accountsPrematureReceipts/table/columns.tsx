...
export const columns = (
  t: TFunction,
  setSelectedTrackingSlip: (selectedRecordId: number) => void,
  ...
  },
  {
    ...
    render: (_, record) => (
      <>
        <Row>
          <Tooltip title="Edit">
            <Space style={{ position: 'absolute', top: 10, left: 0 }}>
              <AntButton
                type="text"
                icon={<EditFilled style={{ color: '#006640' }} />}
                onClick={() => {
                  setSelectedTrackingSlip(record?.TrakingNo), setSelectedRecordId(record?.Id), setActiveTab('2');
                }}
              />
            </Space>
          </Tooltip>
