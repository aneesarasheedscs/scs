...
const CardView: React.FC<{ setSelectedRecordId: (id: number | null) => void; setActiveTab: (tab: string) => void }> = ({
  ...
  const { data } = useGetBankReceiptVoucherTable();
  const voucherData = data?.data?.Data?.Result;
  console.log(data?.data?.Data?.Result);
  const [records, setRecords] = useState<TBankReceiptVoucherTable[]>([]);
  const [selectedCardData, setSelectedCardData] = useState<TBankReceiptVoucherTable>();
  ...
  const handleSearch = (value: any) => {
    ...
    const filteredRecords = voucherData?.filter((record: TBankReceiptVoucherTable) => {
    ...
    <div>
      <Row className="row1">
        <Col lg={12} md={12} xl={7} xxl={6} sm={24} xs={24} className="columns">
          ...
          <div style={{ backgroundColor: ' #d6d6e7', maxHeight: 'calc(100vh - 250px)', overflowY: 'auto' }}>
            <Row
              gutter={[16, 16]}
              style={{
                borderRadius: '1%',
            ...
            >
              {records?.map((card: TBankReceiptVoucherTable | any) => (
