...
import { useGetAccountTitle, useGetCashReceiptVoucherTable } from '../queries/queries';
...
function SearchCriteria() {
  ...
  const {
    data,
    isError,
    isLoading: isLoadingCRV,
    refetch: refetchCRV,
    isFetching,
  } = useGetCashReceiptVoucherTable(true, formValues);

  const handleOpen = () => setOpen(true);
  const handleClose = () => setOpen(false);

  const onFinish = (_: TSearchCriteria) => {
    refetchCRV().then(() => handleClose());
  };
