...
import { useGetAccountTitle, useGetBankPaymentVoucherTable } from '../queries/queries';
...
function SearchCriteria() {
  ...
  const {
    data,
    isError,
    isLoading: isLoadingBPV,
    refetch: refetchBPV,
    isFetching,
  } = useGetBankPaymentVoucherTable(true, formValues);
  const [value, setValue] = useState(1);
  const handleOpen = () => setOpen(true);
  const handleClose = () => setOpen(false);

  const onFinish = (_: TSearchCriteria) => {
    refetchBPV().then(() => handleClose());
  };
