...
const DynamicForm = ({ form }: TDynamicForm) => {
  ...
  const handleUpdateToTable = () => {
    ...
    setTableData((prevData: any[]) => {
      ...
      if (editedRowIndex >= 0) {
        // Update the corresponding item in the data array and the row above it
        const updatedData = prevData.map((row) => {
          if (row.LineId === edit.LineId || row.LineId === edit.LineId - 1) {
            // Update the current row or the row above it
            return {
              ...newData[0], // Update with the first item from newData
              LineId: row.LineId,
            };
          }
