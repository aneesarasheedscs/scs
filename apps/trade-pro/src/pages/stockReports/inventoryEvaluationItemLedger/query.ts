...
export const useGetDocumentType = () => {
  return useQuery('document-type', getDocumentType, {
    cacheTime: userDetail?.expires_in,
  });
};
const getDocumentType: QueryFunction<AxiosResponse<any, any>> = async () => {
  const response = await requestManager.get('/api/InventoryReports/InventoryStockEvalautionDetailDropDownAndLists', {
    params,
  });
  const rawData = response.data?.Data.Result || [];
  const filteredData = rawData.filter((item: any) => item.ActivityType === 'DocumentType');
  console.log(filteredData);
  return filteredData;
};
//Get Stock Account
...
export const useInventoryReportHistory = (enabled?: boolean, params?: TInventoryEvaluationLedgerSearchCriteria) => {
  return useQuery(
    'inventory-report-evaluation-history',
    () => {
